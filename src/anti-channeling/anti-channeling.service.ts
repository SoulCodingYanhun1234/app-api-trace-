...
export class AntiChannelingService {
  ...
  private shipmentDecisionRegion(shipment: any = {}, fallbackLocation?: unknown) {
    // A shipped code is authorized by the shipment origin, never by its receiving agent.
    const senderText = safeText(shipment?.authorization_address || shipment?.sender_address || fallbackLocation, 255);
    const parsedSender = this.parseLocation(senderText);
    ...
    const location = safeText([provinceName, cityName].filter(Boolean).join(' / ') || parsedSender.location || senderText, 255);
    const regionKey = [this.compact(provinceName), this.compact(cityName)].filter(Boolean).join('/');
    return {
      location,
  ...
  private async authorizedRegionForScan(ctx: ScanContext) {
    ...
    if (shipment) {
      const shipmentAuthorization = this.shipmentDecisionRegion(shipment);
      ...
      return {
        ...
        shipment_sender_address: shipmentAuthorization.sender_address || null,
        ...
        authorized_scope: hasShipmentRegion ? 'shipment_sender_location' : 'shipment_sender_location_unresolved',
        source: hasShipmentRegion ? 'shipment_sender_location' : 'shipment_sender_location_unresolved',
      };
    }
   ...
   * 有发货单时一定返回“发货位置”的授权地区；制造商字段仅用于产品身份展示。
   */
  ...
  private async evaluateLocationEvidence(ctx: ScanContext) {
    ...
    if (!authorized.province_name && !authorized.city_name) {
      const unresolvedSources = ['shipment_sender_location_unresolved'];
      ...
      return this.upsertAlert('location_unverified', {
        ...
        evidence: {
          ...
          reason: '已发货防伪码按发货位置授权；当前发货位置缺少可识别省市。',
          shipment_no: authorized.shipment_no || null,
  ...
  async evaluateShipmentPreCheck(body: Record<string, any>) {
    ...
    if (!shipmentRegion.province_name && !shipmentRegion.city_name) {
      throw new BadRequestException('请填写可解析省市的发货位置；出库后防伪码授权位置以该发货位置为准');
    }
