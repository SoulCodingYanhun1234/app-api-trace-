...
export class ResourcesService {
  ...
  private compactAgentAddress(agent: any) {
    return safeText([agent?.province, agent?.city, agent?.district, agent?.address]
      .map((item) => safeText(item, 255))
      .filter(Boolean)
      .join(''), 255);
  }

  /**
   * 已发货后的防窜授权位置来自发货位置（sender_address），而非收货代理商。
   * 装箱地点只记录业务流转，不形成防伪码授权位置。
   */
  private resolveShipmentDestinationLocation(shipment: any = {}, _agent: any = null, fallbackLocation?: unknown) {
    const senderText = safeText(shipment?.authorization_address || shipment?.sender_address || fallbackLocation, 255);
    const parsedSender = this.parseDestination(senderText);
    const isSenderSnapshot = shipment?.authorization_source === 'shipment_sender_location';
    const provinceName = safeText(parsedSender.province_name || (isSenderSnapshot ? shipment?.province_name : ''), 64);
    const cityName = safeText(parsedSender.city_name || (isSenderSnapshot ? shipment?.city_name : ''), 64);
    return {
      raw_address: senderText,
      province_name: provinceName,
      city_name: cityName,
      region_group: safeText([provinceName, cityName].filter(Boolean).join(' / '), 128),
      basis: provinceName || cityName ? 'shipment_sender_location' : 'shipment_sender_location_unresolved',
    };
  }

  resolveShipmentOriginLocation(senderAddress?: unknown) {
    return this.resolveShipmentDestinationLocation({ sender_address: senderAddress });
  }
  ...
  private async hydrateShipmentPayload(payload: Record<string, any>, client: any = this.prisma, currentShipmentId?: number) {
    const next = { ...payload };
    if (!next.agent_id) throw new BadRequestException('请选择发货到达的代理商');
    const agent = await this.agentSnapshot(Number(next.agent_id), client);
    if (!agent) throw new BadRequestException('所选发货代理商不存在');
    this.requireAgentRegion(agent, '发货到达代理商');
    const distributorName = safeText(agent?.agent_name || agent?.agent_code, 128);

    if (agent) {
      if (!next.receiver) next.receiver = safeText(agent.contact_name || distributorName, 64);
      if (!next.receiver_phone) next.receiver_phone = safeText(agent.contact_phone, 32);
      if (!next.receiver_address) next.receiver_address = this.compactAgentAddress(agent);
      next.distributor = distributorName || next.distributor;
    }
    ...
    const authorization = this.shipmentAuthorizationSnapshot(next, agent);
    if (await this.shipmentHasEnabledAntiChannelingCodes(next.box_ids, client)) {
      if (!authorization.raw_address || !authorization.province_name || !authorization.city_name) {
        throw new BadRequestException('箱内存在开启防窜校验的防伪码，请填写可解析省、市的发货位置作为授权位置');
      }
    }
  ...
  private shanghaiDateParts(date: Date = new Date()) {
  ...
  private boxDateCode(date: Date = new Date()) {
    const { month, day } = this.shanghaiDateParts(date);
    return `${String(month).padStart(2, '0')}.${String(day).padStart(2, '0')}`;
  }

  private boxNoPrefix(date: Date = new Date()) {
    return `QRB${this.boxDateCode(date)}-`;
  }

  private boxDayRange(date: Date = new Date()) {
    const { year, month, day } = this.shanghaiDateParts(date);
  ...
  private isCanonicalBoxNo(value: unknown) {
    return /^QRB\d{2}\.\d{2}-[1-9]\d*$/i.test(String(value || '').trim());
  }

  private async nextBoxNo(client: any, date: Date = new Date()) {
    const prefix = this.boxNoPrefix(date);
    // 只读取当天最新记录即可确定最大流水号；唯一索引与创建重试负责并发冲突兜底。
    const rows = await client.box.findMany({
      where: { box_no: { startsWith: prefix }, created_at: this.boxDayRange(date) },
      select: { box_no: true },
      orderBy: { id: 'desc' },
      take: 50000,
    });
    let maxSequence = 0;
    for (const row of rows) {
      const value = String(row?.box_no || '').trim().toUpperCase();
      const match = value.match(/^QRB\d{2}\.\d{2}-(\d+)$/);
      if (!match) continue;
  ...
  private async createBoxWithCascade(payload: Record<string, any>) {
    ...
    if (requestedInput && !this.isCanonicalBoxNo(requestedInput)) {
      throw new BadRequestException('箱码编号格式不规范；请留空由系统生成，格式为 QRB月.日-当天流水号，例如 QRB07.18-1');
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      try {
        const row = await this.prisma.$transaction(async (tx: any) => {
          ...
          data.box_no = requestedNo ? requestedNo.toUpperCase() : await this.nextBoxNo(tx, new Date());
          const created = await tx.box.create({ data });
          await this.syncBoxCodesCascadeTx(tx, created, plaintextCodes);
          return this.hydrateBoxCodeList(created, tx);
        });
  ...
  async update(resource: string, id: string | number, data: Record<string, any>) {
    ...
    if (resource === 'returns') payload = await this.hydrateReturnPayload(payload);
    if (resource === 'shipments') {
      // 通用编辑可能只提交局部字段。先读取原发货单，确保授权快照始终基于已选 distributor 代理商，
      // 而不是因一次物流/备注修改而丢失 agent_id 后回退到收件地址。
      const tableName = this.resourceTableName(resource);
      ...
      if (Number(existing.status || 0) >= 1 && payload.agent_id && Number(payload.agent_id) !== Number(existing.agent_id)) {
        throw new BadRequestException('发货单已出库，不能更换发货代理商；请新建发货单以保留原授权轨迹');
      }
  ...
  async generateCodes(body: Record<string, any>) {
    ...
    const rows = Array.from({ length: count }, () => {
      const code = this.codePolicy.issueOrLegacy(() => `${prefix}${nanoid()}`);
      return {
        __plaintext: code,
        ...productData,
