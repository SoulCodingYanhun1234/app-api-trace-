...
export class ScannerService {
  ...
  private async createAttributedShipment(agentInput: unknown, regionInput: unknown, payload: Record<string, any> = {}) {
    ...
    // agent_id records the receiving party; sender_address is the anti-channeling authorization location.
    const agent = agentInput ? await this.requireAgent(agentInput) : (region?.agent_id ? await this.requireAgent(region.agent_id) : null);
    if (!agent) throw new BadRequestException('请选择发货到达的代理商（Distributor）');
    const senderAddress = String(payload.sender_address || '').trim();
    const authorization = this.resources.resolveShipmentOriginLocation(senderAddress);
    if (!authorization.province_name || !authorization.city_name) {
      throw new BadRequestException('请填写可解析省、市的发货位置，作为防伪码授权位置');
    }
    ...
    for (const key of ['logistics_company', 'logistics_no', 'sender', 'sender_address', 'receiver', 'receiver_phone', 'receiver_address', 'remark']) {
      if (payload[key] !== undefined && payload[key] !== null && payload[key] !== '') data[key] = payload[key];
    }
    if (!data.receiver) data.receiver = agent.contact_name || agent.agent_name || agent.agent_code || null;
    if (!data.receiver_phone) data.receiver_phone = agent.contact_phone || null;
    if (!data.receiver_address) data.receiver_address = [agent.province, agent.city, agent.address].filter(Boolean).join('') || null;
  ...
  private async appendBoxToShipment(shipmentId: number, box: any, payload: Record<string, any> = {}) {
    ...
    if (shipmentData.sender_address) {
      const authorization = this.resources.resolveShipmentOriginLocation(shipmentData.sender_address);
      if (!authorization.province_name || !authorization.city_name) {
        throw new BadRequestException('请填写可解析省、市的发货位置，作为防伪码授权位置');
      }
      shipmentData.authorization_address = authorization.raw_address;
      shipmentData.authorization_level = authorization.city_name ? 'city' : 'province';
      shipmentData.authorization_source = authorization.basis;
      shipmentData.province_name = authorization.province_name;
      shipmentData.city_name = authorization.city_name;
      shipmentData.region_group = authorization.region_group;
    }
