...
import { AntiCounterfeitCodePolicy, type AntiCounterfeitCodeAssessment } from '../common/anti-counterfeit-code.js';
...
export class QueryService {
   ...
   *
   * 签名码策略用于约束新码发行和阻止伪造的 AF1 签名码；如果生产环境
  ...
  async query(data: {
    ...
    const signedFormatCandidate = data.code.startsWith('AF1.');
    ...
    // 历史码仍应支持平滑过渡。带 AF1 前缀但签名无效的码绝不进入兼容分支。
