...
export class TraceabilityV1Service {
  ...
  async generateCodes(body: Record<string, any>) {
    ...
    const rows = Array.from({ length: quantity }, (_, index) => {
      ...
      const code = this.codePolicy.issueOrLegacy(() => `${raw}-${check}`);
      return {
  ...
  async shipmentOut(body: Record<string, any>) {
    ...
    if (!boxes.length && codes.length) {
      const boxNo = this.text(body.autoBoxNo || body.auto_box_no || `BOX${Date.now()}`, 128);
      await this.resources.create('box', { box_no: boxNo, codes, box_capacity: codes.length, status: 1 }).catch((error: any) => this.logger.warn(`自动建箱失败：${error?.message || error}`));
      const box = await (this.prisma as any).box.findFirst({ where: { box_no: boxNo } }).catch(() => null);
